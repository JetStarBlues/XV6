...
/* Retrieved from
   ...
   Don't be mislead by the 24 bits used in this palette!
   24 bits allow us to encode each channel (r,g,b) as a byte.
   This makes it easier to index into the palette to get a
   particular color.

   When we actually send the colors to the VGA, each byte
   will be converted into the 6 bits the VGA expects.
   See 'vgaSetDefaultPalette'. 
*/
...
void vgaSetDefaultPalette ()
{
	...
	for ( index = 0; index < 256; index += 1 )
	{
		...
		// Turn bytes into 6 bits
		vgaSetPalette( 

			index,
			( ( value & 0xff0000 ) >> 16 ) >> 2,  // r
			( ( value & 0x00ff00 ) >> 8  ) >> 2,  // g
			( ( value & 0x0000ff )       ) >> 2   // b
		);
	}  
}
